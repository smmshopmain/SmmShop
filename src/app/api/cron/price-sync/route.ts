...
import { priceSyncTask } from "@/lib/sync-tasks";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const taskPromise = priceSyncTask();
    ...
    if (raced === null) {
      void (async () => {
        try {
          ...
          console.error("Background priceSyncTask failed:", err);
        }
      })();
      return ok({ message: "Price recalculation started (running in background)" });
    }
